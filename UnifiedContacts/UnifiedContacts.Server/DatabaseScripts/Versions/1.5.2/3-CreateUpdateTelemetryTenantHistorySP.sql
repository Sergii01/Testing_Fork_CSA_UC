IF NOT EXISTS (SELECT * FROM [dbo].[SchemaVersions] WHERE [ScriptName] = 'Unified_Contacts.DatabaseScripts.Versions._1._5._2.3-CreateUpdateTelemetryTenantHistorySP.sql')
BEGIN
    ...
    AS
    BEGIN
        DECLARE @HistoryIncrementTable TABLE ([date] date, resultCountAad int, resultCountUserContact int, resultCountOrgContact int, resultCountSharePoint int, resultCountDatabase int, requestCount int, requestCountAndroid int, requestCountIos int, requestCountWeb int, requestCountDesktop int, requestCountUnknown int, activeUserCountLast30Days int)
        ...
        IF @@ROWCOUNT = 0 
        BEGIN 
            INSERT INTO [UnifiedContacts].[TelemetryTenantHistory] SELECT * FROM @HistoryIncrementTable
        END
    END')
END